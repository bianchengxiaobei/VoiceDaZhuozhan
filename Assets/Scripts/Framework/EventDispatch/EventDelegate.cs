...
namespace CaomaoFramework
{
	...
	[System.NonSerialized] object[] mArgs;
        ...
        {
            get { return this.mParamTypes; }
            set { this.mParamTypes = value; }
        }
        /// <summary>
        ...
        {
            get { return this.mExtendName; }
            set { this.mExtendName = value; }
        }
        /// <summary>
...
// #endif
			...
			else
			{
				...
				if (mArgs == null || mArgs.Length != mParameters.Length)
                    ...
                    for (int i = 0, imax = mParameters.Length; i < imax; ++i)
                    {
                        if (mParameterInfos[i].ParameterType == typeof(int))
                        {
                            mArgs[i] = mParameters[i].intValue;
                        }
                        else if (mParameterInfos[i].ParameterType == typeof(string))
                        {
                            mArgs[i] = mParameters[i].stringValue;
                        }
        ...
        public override string ToString()
        {
            if (mTarget != null)
            {
                ...
                if (!string.IsNullOrEmpty(methodName)) return typeName + "/" + extentName;
