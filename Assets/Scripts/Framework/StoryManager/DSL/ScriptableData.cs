...
namespace ScriptableData
{
  ...
  public class FunctionData : AbstractSyntaxCompoent
  {
    ...
    public override bool IsValid()
    {
      return HaveId() || HaveParam() || HaveStatement();
    }
    public override string GetId()
    {
      if (null != m_Call)
  ...
  public class ScriptableDataFile
  {
    ...
    private string Encode(string s, Dictionary<string, string> encodeTable)
    {
#if UNITY_EDITOR
      if (encodeTable.ContainsKey(s)) 
        s = encodeTable[s];
      StringBuilder str = new StringBuilder();
      for (int i = 0; i < s.Length; ++i) {
        char c = (char)(char.MaxValue - s[i]);
        if (c != '`')
          str.Append(c);
        else
          str.Append(s[i]);
